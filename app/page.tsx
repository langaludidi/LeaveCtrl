...
export default async function HomePage() {
  ...
  const [
    { data: leaveTypes }, { data: balances }, { data: toilBalance }, { data: myRequests }, { data: myToilRequests },
    { data: pendingVisible }, { data: pendingToilVisible }, { data: employees }, { data: departments }, { data: currentConditions },
    { data: awayToday }, { data: toilAwayToday }, { data: upcomingApproved }, { data: upcomingToil },
    { data: leaveCoverageWarnings }, { data: toilCoverageWarnings },
  ] = await Promise.all([
    ...
    supabase.from("leave_requests").select("id, employee_id, leave_type_id, start_date, end_date, quantity, status, submitted_at").eq("employee_id", employee.id).order("created_at", { ascending: false }).limit(5),
    supabase.from("toil_requests").select("id, employee_id, leave_date, hours, status, submitted_at").eq("employee_id", employee.id).order("created_at", { ascending: false }).limit(5),
    supabase.from("leave_requests").select("id, employee_id, leave_type_id, start_date, end_date, quantity, status, submitted_at").eq("organisation_id", employee.organisation_id).in("status", ["pending_approval", "cancellation_requested"]).order("submitted_at", { ascending: true }),
    supabase.from("toil_requests").select("id, employee_id, leave_date, hours, status, submitted_at").eq("organisation_id", employee.organisation_id).in("status", ["pending_approval", "cancellation_requested"]).order("submitted_at", { ascending: true }),
    supabase.from("employees").select("id, first_name, last_name, department_id").eq("organisation_id", employee.organisation_id),
    ...
    supabase.from("leave_requests").select("id, employee_id").eq("organisation_id", employee.organisation_id).in("status", ["approved", "cancellation_requested"]).lte("start_date", today).gte("end_date", today),
    supabase.from("toil_requests").select("id, employee_id").eq("organisation_id", employee.organisation_id).in("status", ["approved", "cancellation_requested"]).eq("leave_date", today),
    supabase.from("leave_requests").select("id, employee_id, leave_type_id, start_date, end_date, quantity, status").eq("organisation_id", employee.organisation_id).in("status", ["approved", "cancellation_requested"]).gte("end_date", today).order("start_date", { ascending: true }).limit(8),
    supabase.from("toil_requests").select("id, employee_id, leave_date, hours, status").eq("organisation_id", employee.organisation_id).in("status", ["approved", "cancellation_requested"]).gte("leave_date", today).order("leave_date", { ascending: true }).limit(8),
    supabase.from("leave_request_coverage_checks").select("request_id").eq("organisation_id", employee.organisation_id).eq("outcome", "warning"),
    supabase.from("toil_request_coverage_checks").select("request_id").eq("organisation_id", employee.organisation_id).eq("outcome", "warning"),
  ]);
  ...
  const currentDepartmentMap = new Map((currentConditions ?? []).map((item) => [item.employee_id, item.department_id]));

  const approvals = canApprove ? (pendingVisible ?? []).filter((request) => request.employee_id !== employee.id) : [];
  const toilApprovals = canApprove ? (pendingToilVisible ?? []).filter((request) => request.employee_id !== employee.id) : [];
  const pendingMine = (myRequests ?? []).filter((request) => ["pending_approval", "cancellation_requested"].includes(request.status)).length + (myToilRequests ?? []).filter((request) => ["pending_approval", "cancellation_requested"].includes(request.status)).length;
  const awayCount = new Set([...(awayToday ?? []).map((item) => item.employee_id), ...(toilAwayToday ?? []).map((item) => item.employee_id)]).size;
  ...
  const coverageAlerts = approvals.filter((request) => request.status === "pending_approval" && leaveWarningIds.has(request.id)).length + toilApprovals.filter((request) => request.status === "pending_approval" && toilWarningIds.has(request.id)).length;

  const combinedApprovals = [
    ...approvals.map((request) => ({ kind: "leave" as const, id: request.id, employeeId: request.employee_id, date: request.start_date, duration: `${compactNumber(Number(request.quantity))} ${Number(request.quantity) === 1 ? "day" : "days"}`, label: `${request.status === "cancellation_requested" ? "Cancellation · " : ""}${typeMap.get(request.leave_type_id) ?? "Leave"}${leaveWarningIds.has(request.id) ? " · Coverage warning" : ""}`, status: request.status, submittedAt: request.submitted_at })),
    ...toilApprovals.map((request) => ({ kind: "toil" as const, id: request.id, employeeId: request.employee_id, date: request.leave_date, duration: `${compactNumber(Number(request.hours))} hours`, label: `${request.status === "cancellation_requested" ? "TOIL cancellation" : "TOIL request"}${toilWarningIds.has(request.id) ? " · Coverage warning" : ""}`, status: request.status, submittedAt: request.submitted_at })),
  ].sort((a, b) => (a.submittedAt ?? "").localeCompare(b.submittedAt ?? ""));

  const upcomingAbsences = [
    ...(upcomingApproved ?? []).map((request) => ({ id: `leave:${request.id}`, employeeId: request.employee_id, type: typeMap.get(request.leave_type_id) ?? "Away", start: request.start_date, end: request.end_date, duration: `${compactNumber(Number(request.quantity))} d` })),
    ...(upcomingToil ?? []).map((request) => ({ id: `toil:${request.id}`, employeeId: request.employee_id, type: "TOIL", start: request.leave_date, end: request.leave_date, duration: `${compactNumber(Number(request.hours))} h` })),
  ...
  return (
    <AppShell displayName={displayName} role={roleLabel(roles)} requestCount={approvalCount}>
      ...
      <section className="two-col">
        ...
        {canApprove ? <div className="card approvals-card"><div className="card-title"><h2>Approvals / My Work</h2><Link href="/requests">View all</Link></div><div className="approval-list">
          {combinedApprovals.slice(0, 3).map((request) => { const person = employeeMap.get(request.employeeId); const name = person ? `${person.first_name} ${person.last_name}` : "Employee"; const initials = name.split(" ").map((value) => value[0]).slice(0, 2).join(""); return <div className="approval-row" key={`${request.kind}:${request.id}`}><div className="mini-avatar">{initials}</div><div className="approval-person"><strong>{name}</strong><span>{request.label}</span></div><div className="approval-date"><strong>{formatDate(request.date)}</strong><span>{request.duration}</span></div><div className="approval-review-actions"><Link href={request.kind === "leave" ? `/requests/leave/${request.id}` : `/requests/toil/${request.id}`} className="request-text-action">Review</Link>{request.kind === "leave" ? <DecisionButtons requestId={request.id} kind={request.status === "cancellation_requested" ? "cancellation" : "leave"}/> : <ToilDecisionButtons requestId={request.id} kind={request.status === "cancellation_requested" ? "cancellation" : "request"}/>}</div></div>; })}
